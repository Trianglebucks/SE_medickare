<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $checklogin = mysqli_query($db, "SELECT * FROM admin WHERE adminEmail = '$email' AND adminPassword = '$password'");
        if(mysqli_num_rows($checklogin) > 0) {
            $stmt = $db->prepare("SELECT * FROM admin WHERE adminEmail = ? AND adminPassword = ? limit 1");
            $stmt->bind_param('ss', $email, $password);
            $stmt->execute();
            $result = $stmt->get_result();
            $value = $result->fetch_object();

            $_SESSION['firstname'] = $value->adminFirstName;
            $_SESSION['lastname'] = $value->adminLastName;
            $_SESSION['email'] = $value->adminEmail;
            $_SESSION['password'] = $value->adminPassword;
