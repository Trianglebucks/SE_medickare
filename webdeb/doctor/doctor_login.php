<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $checklogin = mysqli_query($db, "SELECT * FROM doctor WHERE doctorEmail = '$email' AND doctorPassword = '$password'");
        if(mysqli_num_rows($checklogin) > 0) {
            $stmt = $db->prepare("SELECT * FROM doctor WHERE doctorEmail = ? AND doctorPassword = ? limit 1");
            $stmt->bind_param('ss', $email, $password);
            $stmt->execute();
            $result = $stmt->get_result();
            $value = $result->fetch_object();

            $_SESSION['firstname'] = $value->doctorFirstName;
            $_SESSION['lastname'] = $value->doctorLastName;
            $_SESSION['email'] = $value->doctorEmail;
            $_SESSION['password'] = $value->doctorPassword;
            $_SESSION['specialization'] = $value->doctorSpecialization;
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="style_doctor_login.css">
    </head>
    <body>
        ...
        <div class="container d-flex justify-content-center">
            <div class="row">
                <div>
                    <br><br>
                    <center class="display-6" style="font-weight: 500;">DOCTOR LOGIN FORM</center>
                    <br>
